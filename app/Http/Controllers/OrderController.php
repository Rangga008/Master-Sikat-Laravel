    ...
    try {
        ...
        if ($validatedData['action'] === 'reject') {
            $order->update([
                ...
                'rejection_notes' => $validatedData['notes'] ?? 'Pembayaran ditolak'
            ]);
            ...
            return redirect()->route('restaurant.orders.index')
                ->with('success', 'Pembayaran DITOLAK dan pesanan dibatalkan');
        }
...
public function updateStatus(Request $request, Order $order)
{
    // Validasi input
    $validatedData = $request->validate([
        'status' => 'required|in:cancelled,processing,completed'
    ]);

    // Pastikan hanya pemilik pesanan yang bisa mengupdate status
    ...
    if ($validatedData['status'] === 'cancelled') {
        // Hanya bisa dibatalkan jika dalam status pending atau unpaid
        ...
        // Update status pembayaran hanya jika status saat ini adalah unpaid
        if ($order->payment_status === 'unpaid') {
            $order->payment_status = 'cancelled'; // Pastikan ini diupdate
        }
    }
...
public function canBeCancelled()
{
    ...
    return $hoursSinceOrder <= 1 && 
           in_array($this->status, ['pending', 'unpaid']) && 
...
public function restaurantOrderIndex(Request $request)
{
    ...
    // Ambil produk yang dimiliki oleh pengguna saat ini
    $userProducts = auth()->user()->products()->pluck('id');

    // Filter berdasarkan produk yang dimiliki
    $query->whereHas('items.product', function ($q) use ($userProducts) {
        $q->whereIn('id', $userProducts);
    });
