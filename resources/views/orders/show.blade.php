<x-app-layout>
    ...
    <div class="bg-white shadow-lg rounded-lg p-6 mt-6">
        ...
        <div class="bg-blue-100 border-l-4 border-blue-500 p-4 mb-4">
            ...
            <p>Nama Pemilik Rekening: <strong>{{ auth()->user()->bank_account_name }}</strong></p>
            <p>Nomor Rekening: <strong>{{ auth()->user()->bank_account_number }}</strong></p>
        </div>
        
        <form action="{{ route('orders.upload-payment-proof', $order) }}" method="POST" enctype="multipart/form-data">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block mb-2">Nama Bank</label>
                    <input type="text" name="bank_name" class="w-full border rounded p-2" required placeholder="Contoh: BCA, Mandiri, dll">
                </div>
                
                <div>
                    <label class="block mb-2">Nama Pemilik Rekening</label>
                    <input type="text" name="account_name" class="w-full border rounded p-2" required placeholder="Nama sesuai rekening">
                </div>
                
                <div>
                    <label class="block mb-2">Tanggal Transfer</label>
                    <input type="date" name="transfer_date" class="w-full border rounded p-2" required>
                </div>
                
                <div>
                    <label class="block mb-2">Bukti Transfer (Maks 2MB)</label>
                    <input type="file" name="payment_proof" accept="image/*" class="w-full border rounded p-2" required>
                </div>
            </div>

            <div class="mt-4 flex space-x-4">
                <button type="submit" class="w-1/2 bg-green-500 text-white py-2 rounded hover:bg-green-600">
                    Upload Bukti Pembayaran
                </button>

                {{-- Tombol Batalkan Pesanan --}}
@if(in_array($order->status, ['pending', 'unpaid']) && $order->payment_status !== 'paid')
<div class="mt-4">
